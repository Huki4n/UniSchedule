...
- Таблицы `Lessons`, `Settings`, `NotificationLog`, `Homework`, `HomeworkComment`, `SubjectRollback`. Ключи настроек: `SelectedGroup`, `SemesterStart` (`yyyy-MM-dd`), `ReminderMinutes` (числа через запятую; пусто — напоминаний нет), `NotificationsEnabled`, `Autostart`, `MinimizeToTray` (`1`/`0`). Если `ReminderMinutes` нет, читаются `FirstReminderMinutes` и `SecondReminderMinutes`. `Homework.Deadline` — `yyyy-MM-dd`, `IsDone` — `1`/`0`, `Url` и `ExtraUrl` — текст. `HomeworkComment.CreatedAt` — `yyyy-MM-dd HH:mm`. `SubjectRollback` хранит первое название пары за календарный день: `LessonId`, `OriginalSubject`, `ChangedOn` (`yyyy-MM-dd`).
